...
import {
    User2, Tags, UploadCloud, Pencil, LogOut, Trash2, Info,
} from "lucide-react";
...
function ProfilePage() {
    ...
    useEffect(() => {
        fetch(`http://localhost:5000/profile/${userId}`)
            ...
            .then(data => {
                setProfile(data);
                // If your profile data has tags assigned, set them here
    ...
    // Optional: Save selected tags back to backend (you can add a Save button and handler)

    if (loading) return <div>Loading profile...</div>;
    if (!profile) return <div>Profile not found or deleted.</div>;
        ...
        <div className="max-w-5xl mx-auto px-6 py-10">
            ...
            <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-2">
                    {tags.length === 0 && <p className="text-gray-500">No tags available.</p>}
                    {tags.map(tag => (
                        <button
                            key={tag}
                            onClick={() => toggleTag(tag)}
                            className={selectedTags.includes(tag) ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700"}
                        >
                            {tag}
                        </button>
                    ))}
            ...
            <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <ProfileOption icon={<UploadCloud />} label="Upload Profile Picture" onClick={() => {
                    // your existing handleUpload function here
                ...
                <ProfileOption icon={<Pencil />} label="Edit Info" onClick={() => {
                    // existing handleEdit
                ...
                <ProfileOption icon={<LogOut />} label="Logout" color="text-red-600" onClick={() => alert('Logged Out!')} />
                <ProfileOption icon={<Trash2 />} label="Delete Profile" color="text-red-600" onClick={() => {
